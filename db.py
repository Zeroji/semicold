...
def process(client, message):
    ...
    Master=None
    for m in client.get_all_members():
        if m.id=='111100569845784576': Master=m
    ...
    if private and ID==client.user.id: bot=True
...
After setup is complete you can use ;afk and ;back''')
    ...
    if C=='isprm':
        ...
        if n<=primes[-1]:
            ...
            if k<0: send('False')
                ...
                k+=1
                s='th'
                if k%10==1 and k!=11: s='st'
                if k%10==2 and k!=12: s='nd'
                if k%10==3 and k!=13: s='rd'
                send('True ('+str(k)+s+')')
    ...
    # Say
    if C=='say' and ranked and not bot: sendT(T)
